...
import com.njupt.safe.engine.AppInfoService;
import com.njupt.safe.utils.DensityUtil;
import com.njupt.safe.view.AppManagerAdapter;
...
//		rl_loading = (RelativeLayout) findViewById(R.id.rl_loading);
		dialog = new LoadingDialog(this);//自定义loading
		dialog.setCancelable(false);//解决单击dialog时dialog消失的问题
...
//		rl_loading.setVisibility(View.VISIBLE);
	...
	private final class MyOnItemClickListener implements OnItemClickListener {
		...
		public void onItemClick(AdapterView<?> parent, View view, int position,
			...
			LinearLayout ll_popup_window = (LinearLayout) contentView
					.findViewById(R.id.ll_popup_window);
			...
			mPopupWindow = new PopupWindow(contentView,
					ViewGroup.LayoutParams.WRAP_CONTENT, DensityUtil.px2dip(getApplicationContext(), 70));
	...
	private final class MyOnClickListener implements OnClickListener {
		...
		public void onClick(View v) {
			...
			case R.id.ll_start:
				try {
					PackageInfo packgeInfo = pm.getPackageInfo(packageName,
							PackageManager.GET_ACTIVITIES);
					ActivityInfo[] activites = packgeInfo.activities;
					if (activites == null || activites.length == 0) {
						Toast.makeText(getApplicationContext(), "该应用程序不能被启动", 0)
								.show();
					} else {
			...
			case R.id.ll_share:
				...
				share_intent.putExtra(Intent.EXTRA_TEXT, "以下是超级大帅哥俊东大神的在测试app的信息:HI, 推荐你使用一款有超级大帅哥俊东大神开发的app: " + appInfo.getApp_name());
	...
	public void onClick(View v) {
		...
		case R.id.tv_title:
			if (isAllApp) {
				mAdapter.setAppInfos(userAppInfos);
				mAdapter.notifyDataSetChanged();
				tv_title.setText("用户程序");
				isAllApp = false;
			} else {
				mAdapter.setAppInfos(appInfos);
				mAdapter.notifyDataSetChanged();
				tv_title.setText("所有程序");
				isAllApp = true;
			}
			break;
