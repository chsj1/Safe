...
import com.njupt.safe.bean.Contact;
import com.njupt.safe.bean.ContactInfo;
import com.njupt.safe.view.ContactAdapter;
 ...
 * @author guolin
 */
public class ContactList1Activity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.contact_list1);
		
		sp = getSharedPreferences("config", Context.MODE_PRIVATE);//用来解决数据无法回传的问题
		
		adapter = new ContactAdapter(this, R.layout.contact_item1, contacts);
		...
		Cursor cursor = getContentResolver().query(uri,
				new String[] { "display_name", "sort_key" ,"data1"}, null, null, "sort_key");
		...
		contactsListView.setOnItemClickListener(new MyOnItemClickListener());
	}
	...
	private final class MyOnItemClickListener implements OnItemClickListener{
		...
		public void onItemClick(AdapterView<?> parent, View view, int position,
				long id) {
			Contact info = (Contact) adapter.getItem(position);
			String number = info.getNumber();
			
			System.out.println("------->现在在联系人狂设置number: "+ number);
