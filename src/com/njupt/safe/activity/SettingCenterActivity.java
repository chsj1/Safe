...
import com.njupt.safe.engine.AppLockService;
import com.njupt.safe.engine.BlackNumberService;
import com.njupt.safe.engine.Const;
import com.njupt.safe.engine.SavePreference;
import com.njupt.safe.engine.ShakeShowLocationService;
import com.njupt.safe.engine.ShowAddressService;
...
public class SettingCenterActivity extends Activity implements OnClickListener{
	...
	private CheckBox cb_setting_center_blacknumber_call;
	private CheckBox cb_setting_ceneter_app_lock;
	
	//***新增摇一摇显示当前位置功能
	private CheckBox cb_setting_ceneter_shake_show_location;
	
	//新增的语音信箱的功能用到的变量
	private CheckBox cb_setting_ceneter_voice_sms;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		cb_setting_center_blacknumber_call = (CheckBox) findViewById(R.id.cb_setting_center_blacknumber_call);
		cb_setting_center_blacknumber_call.setOnClickListener(this);
		boolean isblacknumber = sp.getBoolean("isblacknumber", false); 
		if(isblacknumber){
			cb_setting_center_blacknumber_call.setChecked(true);
			cb_setting_center_blacknumber_call.setText("来电黑名单已开启");
			Intent service = new Intent(this,BlackNumberService.class);
			startService(service);
		}else{
			cb_setting_center_blacknumber_call.setChecked(false);
			cb_setting_center_blacknumber_call.setText("来电黑名单已关闭");
		} 
		...
		cb_setting_ceneter_shake_show_location = (CheckBox) findViewById(R.id.cb_setting_ceneter_shake_show_location);
		cb_setting_ceneter_shake_show_location.setOnClickListener(this);
		boolean isShakeShowLocation = sp.getBoolean("isShakeShowLocation", false);
		if(isShakeShowLocation){
			cb_setting_ceneter_shake_show_location.setChecked(true);
			cb_setting_ceneter_shake_show_location.setText("服务已开启");
			Intent service = new Intent(this,ShakeShowLocationService.class);
			startService(service);
		}else{
			cb_setting_ceneter_shake_show_location.setChecked(false);
			cb_setting_ceneter_shake_show_location.setText("服务已关闭");
			Intent service = new Intent(this,ShakeShowLocationService.class);
			stopService(service);
		}
		
		cb_setting_ceneter_voice_sms = (CheckBox) findViewById(R.id.cb_setting_ceneter_voice_sms);
	    cb_setting_ceneter_voice_sms.setOnClickListener(this);
	    boolean isVoiceSms = sp.getBoolean("isVoiceSms", false);
		if(isVoiceSms){
			cb_setting_ceneter_voice_sms.setChecked(true);
			cb_setting_ceneter_voice_sms.setText("语音信箱已开启");
		}else{
			cb_setting_ceneter_shake_show_location.setChecked(false);
			cb_setting_ceneter_shake_show_location.setText("语音信箱已关闭");
		}
	}
	...
	public void onClick(View v) {
		...
		case R.id.cb_setting_center_blacknumber_call:
			boolean isblacknumber = sp.getBoolean("isblacknumber", false);
			if(isblacknumber){
				cb_setting_center_blacknumber_call.setChecked(false);
				cb_setting_center_blacknumber_call.setText("来电黑名单已关闭");
		...
		case R.id.cb_setting_ceneter_shake_show_location:
			//isShakeShowLocation 摇一摇显示当前位置功能...
			boolean isShakeShowLocation = sp.getBoolean("isShakeShowLocation", false);
			if(isShakeShowLocation){
				cb_setting_ceneter_shake_show_location.setChecked(false);
				cb_setting_ceneter_shake_show_location.setText("服务已关闭");
				Editor editor = sp.edit();
				editor.putBoolean("isShakeShowLocation", false);
				editor.commit();
				Intent service = new Intent(this,ShakeShowLocationService.class);
				stopService(service);
			}else{
				...
				cb_setting_ceneter_shake_show_location.setText("服务已开启");
				Editor editor = sp.edit();
				editor.putBoolean("isShakeShowLocation", true);
				editor.commit();
				Intent service = new Intent(this,ShakeShowLocationService.class);
				startService(service);
			}
		...
		case R.id.cb_setting_ceneter_voice_sms:
			//isShakeShowLocation 摇一摇显示当前位置功能...
			boolean isVoiceSms= sp.getBoolean("isVoiceSms", false);
			if(isVoiceSms){
				cb_setting_ceneter_voice_sms.setChecked(false);
				cb_setting_ceneter_voice_sms.setText("语音信箱已关闭");
				Editor editor = sp.edit();
				editor.putBoolean("isVoiceSms", false);
				editor.commit();
			}else{
				cb_setting_ceneter_voice_sms.setChecked(true);
				cb_setting_ceneter_voice_sms.setText("语音信箱已开启");
				Editor editor = sp.edit();
				editor.putBoolean("isVoiceSms", true);
				editor.commit();
			}
			break;
